...
function theme_doctors_init()
{
	...
	register_taxonomy("doctors_category", array("doctors"), array("label" => "Speciality", "singular_label" => 

"Category", 	"rewrite" => true, "hierarchical" => true));
...
function doctor_campus() {
	$labels = array(
		'name'              => _x( 'Campus Categories', 'taxonomy general name' ),
		'singular_name'     => _x( 'Campus Category', 'taxonomy singular name' ),
		'search_items'      => __( 'Search Campus Categories' ),
		'all_items'         => __( 'All Campus Categories' ),
		'parent_item'       => __( 'Parent Campus Category' ),
		'parent_item_colon' => __( 'Parent Campus Category:' ),
		'edit_item'         => __( 'Edit Campus Category' ), 
		'update_item'       => __( 'Update Campus Category' ),
		'add_new_item'      => __( 'Add New Campus Category' ),
		'new_item_name'     => __( 'New Campus Category' ),
		'menu_name'         => __( 'Campus' ),
	);
	$args = array(
		'labels' => $labels,
	...
	register_taxonomy( 'campus_category', 'doctors', $args );
}
add_action( 'init', 'doctor_campus', 0 );


//custom post type - gender
function doctor_gender() {
	$labels = array(
		'name'              => _x( 'Gender', 'taxonomy general name' ),
		'singular_name'     => _x( 'Gender Category', 'taxonomy singular name' ),
		'search_items'      => __( 'Search Gender Categories' ),
		'all_items'         => __( 'All Gender Categories' ),
		'parent_item'       => __( 'Parent Gender Category' ),
		'parent_item_colon' => __( 'Parent Gender Category:' ),
		'edit_item'         => __( 'Edit Gender Category' ), 
		'update_item'       => __( 'Update Gender Category' ),
		'add_new_item'      => __( 'Add New Gender Category' ),
		'new_item_name'     => __( 'New Gender Category' ),
		'menu_name'         => __( 'Gender' ),
	);
	$args = array(
		'labels' => $labels,
	...
	register_taxonomy( 'gender_category', 'doctors', $args );
}
add_action( 'init', 'doctor_gender', 0 );


//custom post type - language
function doctor_language() {
	$labels = array(
	...
	register_taxonomy( 'language_category', 'doctors', $args );
}
add_action( 'init', 'doctor_language', 0 );
...
function theme_inner_doctors_custom_box_main($post)
{
	...
	<table>
		<tr>
			...
			<td>
				<input class="regular-text" type="text" id="doctor_subtitle" 

name="doctor_subtitle" value="' . esc_attr(get_post_meta($post->ID, "subtitle", true)) . '" />
			</td>
		</tr>
		<tr>
			<td>
			...
			<td>
				<input class="regular-text" type="text" id="image_title" name="image_title" 

value="' . esc_attr(get_post_meta($post->ID, "image_title", true)) . '" />
			</td>
		</tr>
		<tr>
			<td>
				<label for="doctors_video_url">' . __('Video URL (optional)', 'medicenter') . 

':</label>
			</td>
			<td>
				<input class="regular-text" type="text" id="doctors_video_url" 

name="doctors_video_url" value="' . esc_attr(get_post_meta($post->ID, "video_url", true)) . '" />
				<span class="description">' . __('For Vimeo please use 

http://player.vimeo.com/video/%video_id% For YouTube: http://youtube.com/embed/%video_id%', 'medicenter') . 
...
'</span>
		...
		<tr>
			<td>
				<label for="doctors_iframe_url">' . __('Ifame URL (optional)', 'medicenter') . 

':</label>
			</td>
			<td>
				<input class="regular-text" type="text" id="doctors_iframe_url" 

name="doctors_iframe_url" value="' . esc_attr(get_post_meta($post->ID, "iframe_url", true)) . '" />
			</td>
		</tr>
		<tr>
			<td>
				<label for="doctors_external_url">' . __('External URL (optional)', 'medicenter') 

. ':</label>
			</td>
			<td>
				<input class="regular-text" type="text" id="doctors_external_url" 

name="doctors_external_url" value="' . esc_attr(get_post_meta($post->ID, "external_url", true)) . '" />
			</td>
		</tr>
		<tr>
			<td>
				<label for="doctors_external_url_target">' . __('External URL target', 

'medicenter') . ':</label>
			</td>
			<td>
				<select id="doctors_external_url_target" name="doctors_external_url_target">
					<option value="same_window"' . ($external_url_target=="same_window" ? ' 

selected="selected"' : '') . '>' . __('same window', 'medicenter') . '</option>
					<option value="new_window"' . ($external_url_target=="new_window" ? ' 

selected="selected"' : '') . '>' . __('new window', 'medicenter') . '</option>
				</select>
			</td>
		</tr>
		<tr>
			<td>
				<label for="doctor_timetable_page">' . __('Timetable page', 'medicenter') . 
...
($timetable_page==get_the_ID() ? ' selected="selected"' : '') . '>' . get_the_title() . '</option>';
				endwhile;
				endif;
				wp_reset_query();
	echo '</td>
			...
			<tr class="repeated_row_id_1 repeated_row_' . ($i+1) . '">
				<td colspan="2">
					<table>
						<tr>
							<td>
								<label>' . __('Icon type', 'medicenter') . " " . 

($i+1) . ':</label>
							</td>
							<td>
								<select id="icon_type_' . ($i+1) . '" 

name="icon_type[]">
									<option value="">-</option>';
									for($j=0; $j<count($icons); $j++)
									{
									echo '<option value="' . esc_attr($icons

[$j]) . '"' . ($icons[$j]==$icon_type[$i] ? " selected='selected'" : "") . '>' . $icons[$j] . '</option>';
									}
							echo '</select>
							</td>
						</tr>
...
esc_attr($icon_url[$i]) . '" name="icon_url[]">
							</td>
						</tr>
						<tr>
							<td>
								<label>' . __('Icon target', 'medicenter') . " " . 
...
($icon_target[$i]=="same_window" ? " selected='selected'" : "") . '>' . __('same window', 'medicenter') . 

'</option>
									<option value="new_window"' . 

($icon_target[$i]=="new_window" ? " selected='selected'" : "") . '>' . __('new window', 'medicenter') . '</option>
								</select>
							</td>
						</tr>
						<tr>
							<td>
								<label>' . __('Icon color', 'medicenter') . " " . 
...
($icon_color[$i]=="blue_light" ? ' selected="selected"' : '') . '>' . __('light blue', 'medicenter') . '</option>
									<option value="blue"' . ($icon_color[$i]

=="blue" ? ' selected="selected"' : '') . '>' . __('blue', 'medicenter') . '</option>
									<option value="blue_dark"' . ($icon_color

[$i]=="blue_dark" ? ' selected="selected"' : '') . '>' . __('dark blue', 'medicenter') . '</option>
									<option value="black"' . ($icon_color[$i]

=="black" ? ' selected="selected"' : '') . '>' . __('black', 'medicenter') . '</option>
									<option value="gray"' . ($icon_color[$i]

=="gray" ? ' selected="selected"' : '') . '>' . __('gray', 'medicenter') . '</option>
									<option value="gray_dark"' . ($icon_color

[$i]=="gray_dark" ? ' selected="selected"' : '') . '>' . __('dark gray', 'medicenter') . '</option>
									<option value="gray_light"' . 

($icon_color[$i]=="gray_light" ? ' selected="selected"' : '') . '>' . __('light gray', 'medicenter') . '</option>
									<option value="green"' . ($icon_color[$i]

=="green" ? ' selected="selected"' : '') . '>' . __('green', 'medicenter') . '</option>
									<option value="green_dark"' . 

($icon_color[$i]=="green_dark" ? ' selected="selected"' : '') . '>' . __('dark green', 'medicenter') . '</option>
									<option value="green_light"' . 

($icon_color[$i]=="green_light" ? ' selected="selected"' : '') . '>' . __('light green', 'medicenter') . 

'</option>
									<option value="orange"' . ($icon_color

[$i]=="orange" ? ' selected="selected"' : '') . '>' . __('orange', 'medicenter') . '</option>
									<option value="orange_dark"' . 

($icon_color[$i]=="orange_dark" ? ' selected="selected"' : '') . '>' . __('dark orange', 'medicenter') . 

'</option>
									<option value="orange_light"' . 

($icon_color[$i]=="orange_light" ? ' selected="selected"' : '') . '>' . __('light orange', 'medicenter') . 

'</option>
									<option value="red"' . ($icon_color[$i]

=="red" ? ' selected="selected"' : '') . '>' . __('red', 'medicenter') . '</option>
									<option value="red_dark"' . ($icon_color

[$i]=="red_dark" ? ' selected="selected"' : '') . '>' . __('dark red', 'medicenter') . '</option>
									<option value="red_light"' . ($icon_color

[$i]=="red_light" ? ' selected="selected"' : '') . '>' . __('light red', 'medicenter') . '</option>
									<option value="turquoise"' . ($icon_color

[$i]=="turquoise" ? ' selected="selected"' : '') . '>' . __('turquoise', 'medicenter') . '</option>
									<option value="turquoise_dark"' . 

($icon_color[$i]=="turquoise_dark" ? ' selected="selected"' : '') . '>' . __('dark turquoise', 'medicenter') . 

'</option>
									<option value="turquoise_light"' . 

($icon_color[$i]=="turquoise_light" ? ' selected="selected"' : '') . '>' . __('light turquoise', 'medicenter') . 

'</option>
									<option value="violet"' . ($icon_color

[$i]=="violet" ? ' selected="selected"' : '') . '>' . __('violet', 'medicenter') . '</option>
									<option value="violet_dark"' . 

($icon_color[$i]=="violet_dark" ? ' selected="selected"' : '') . '>' . __('dark violet', 'medicenter') . 

'</option>
									<option value="violet_light"' . 

($icon_color[$i]=="violet_light" ? ' selected="selected"' : '') . '>' . __('light violet', 'medicenter') . 

'</option>
									<option value="white"' . ($icon_color[$i]

=="white" ? ' selected="selected"' : '') . '>' . __('white', 'medicenter') . '</option>
									<option value="yellow"' . ($icon_color

[$i]=="yellow" ? ' selected="selected"' : '') . '>' . __('yellow', 'medicenter') . '</option>
								</select>
							</td>
						</tr>
					</table>
			...
			<tr>
				<td colspan="2">
					<input type="button" class="button ' . $themename . 

'_add_new_repeated_row" name="' . $themename . '_add_new_repeated_row" id="repeated_row_id_1" value="' . __('Add 
				...
				<td colspan="2">
					<table>
						<tr class="image_url_row">
							<td>
								<label>' . __('Image url', 'medicenter') . " " . 

($i+1) . '</label>
							</td>
							<td>
								<input type="hidden" name="attachment_ids[]" id="' 

. $themename . '_attachment_id_' . ($i+1) . '" value="' . esc_attr($attachment_ids[$i]) . '" />
								<input class="regular-text" type="text" id="' . 

$themename . '_image_url_' . ($i+1) . '" name="images[]" value="' . esc_attr($images[$i]) . '" />
								<input type="button" class="button" name="' . 

$themename . '_upload_button" id="' . $themename . '_image_url_button_' . ($i+1) . '" value="' . __('Browse', 
...
'medicenter') . '" />
						...
						<tr class="image_title_row">
							<td>
								<label>' . __('Image description', 'medicenter') . 
...
$themename . '_image_title_' . ($i+1) . '" name="images_titles[]" value="' . esc_attr($images_titles[$i]) . '" />
							</td>
						</tr>
						<tr class="video_row">
							<td>
								<label>' . __('Video url', 'medicenter') . " " . 
...
$themename . '_video_' . ($i+1) . '" name="videos[]" value="' . esc_attr($videos[$i]) . '" />
							</td>
						</tr>
						<tr class="iframe_row">
							<td>
								<label>' . __('Iframe url', 'medicenter') . " " . 
...
$themename . '_iframe_' . ($i+1) . '" name="iframes[]" value="' . esc_attr($iframes[$i]) . '" />
							</td>
						</tr>
						<tr class="external_url_row">
							<td>
								<label>' . __('External url', 'medicenter') . " " 
...
$themename . '_external_url_' . ($i+1) . '" name="external_urls[]" value="' . esc_attr($external_urls[$i]) . '" />
							</td>
						</tr>
					</table>
					<br />
...
'_add_new_repeated_row" name="' . $themename . '_add_new_repeated_row" id="repeated_row_id_2" value="' . __('Add 
...
image', 'medicenter') . '" />
			...
			<tr>
				<td>
					<br />
					<label for="features_images_loop">' . __('Featured images lightbox loop', 

'medicenter') . ':</label>
				</td>
				<td>
					<select id="features_images_loop" name="features_images_loop">
						<option value="yes"' . ($features_images_loop=="yes" ? ' 

selected="selected"' : '') . '>' . __('yes', 'medicenter') . '</option>
						<option value="no"' . ($features_images_loop=="no" ? ' 

selected="selected"' : '') . '>' . __('no', 'medicenter') . '</option>
					</select>
				</td>
			</tr>
		</table>
...
function doctors_edit_columns($columns)
{
	$columns = array(
			...
			"doctors_category" => __('Speciality', 'medicenter'),
			"gender_category" => __('Gender', 'medicenter'),
			"language_category" => __('Language', 'medicenter'),
			"campus_category" => __('Campus', 'medicenter'),
...
function manage_doctors_posts_custom_column($column)
{
	...
	switch ($column)
	{
		...
		case "gender_category":
			echo get_the_term_list($post->ID, "gender_category", '', ', ','');
			break;
			
		case "language_category":
			echo get_the_term_list($post->ID, "language_category", '', ', ','');
			break;
			
		case "campus_category":
			echo get_the_term_list($post->ID, "campus_category", '', ', ','');
			break;
	}
	...
	wpb_map( array(
		...
		"params" => array(
			...
			array(
				...
				"param_name" => "order_by",
				"value" => array(__("Title, menu order", 'medicenter') => "title,menu_order", __

("Menu order", 'medicenter') => "menu_order")
			),
			array(
				"type" => "dropdown",
				"class" => "",
				...
				"param_name" => "order",
				"value" => array(__("ascending", 'medicenter') => "ASC", __("descending", 
...
'medicenter') => "DESC")
			...
			array(
				...
				"param_name" => "type",
				"value" => array(__("List with details", 'medicenter') => "list_with_details", __

("List", 'medicenter') => "list", __("Details", 'medicenter') => "details")
			),
			array(
				"type" => "dropdown",
				"class" => "",
				...
				"param_name" => "layout",
				"value" => array(__("4 columns", 'medicenter') => "gallery_4_columns", __("2 

columns", 'medicenter') => "gallery_2_columns", __("3 columns", 'medicenter') => "gallery_3_columns")
			),
			array(
				"type" => "dropdown",
				"class" => "",
			...
			array(
				...
				"value" => array(__("Filters", 'medicenter') => 'dm_filters', __("Carousel", 

'medicenter') => 'dm_carousel', __("Pagination", 'medicenter') => 'dm_pagination', __("Simple", 'medicenter') => 
...
'list'))
			...
			array(
				...
				"param_name" => "id",
				"value" => "carousel",
				"description" => __("Please provide unique id for each carousel on the same 

page/post", 'medicenter'),
...
'details'))
			...
			array(
				...
				"value" => array(__("H2", 'medicenter') => "h2", __("H1", 'medicenter') => "h1", 

__("H3", 'medicenter') => "h3", __("H4", 'medicenter') => "h4", __("H5", 'medicenter') => "h5"),
				"dependency" => Array('element' => "type", 'value' => array('list_with_details', 
...
'list'))
			...
			array(
				...
				"value" => array(__("Section (large)", 'medicenter') => "page_margin_top_section", 

__("Page (small)", 'medicenter') => "page_margin_top", __("None", 'medicenter') => "none")
			)
		)
	));
}
